//  Estilos Generales
export const BotonClasico =
  "px-6 py-2.5 font-medium text-xs leading-tight uppercase shadow-md transition hover:scale-105 duration-150 ease-in-out hover:shadow-lg active:shadow-lg  focus:outline-none hover:duration-200 cursor-pointer";

export const BotonClasicoSinZoom =
  "px-6 py-2.5 font-medium text-xs leading-tight uppercase shadow-md transition duration-150 ease-in-out hover:shadow-lg active:shadow-lg  focus:outline-none hover:duration-200 cursor-pointer";

export const BotonRedondeado =
  "px-6 py-2.5  hover:scale-105 font-medium text-xs leading-tight uppercase shadow-md transition duration-150 ease-in-out hover:shadow-lg active:shadow-lg rounded-sm focus:outline-none hover:duration-200  cursor-pointer";

// Colores Generales
export const ColorRojo =
  "bg-red-500  active:bg-red-700 hover:bg-red-600 focus:ring-red-900 text-white cursor-pointer";

export const ColorPrimary =
  "bg-slate-500  focus:bg-slate-400 hover:bg-slate-600 active:bg-slate-700 text-white cursor-pointer";

export const ColorBlanco =
  "bg-white focus:bg-slate-400 hover:bg-slate-600 active:bg-slate-700 text-black cursor-pointer border border-slate-500 hover:text-white";

export const ColorAzul =
  "bg-blue-600  hover:bg-blue-700 focus:ring-o active:bg-blue-800  text-white cursor-pointer ";

export const ColorVerde =
  "bg-green-600 hover:bg-green-700 focus:bg-green-800 active:bg-green-500 text-white cursor-pointer";

export const ColorNegro =
  "bg-black hover:bg-white hover:border hover:border-black focus:bg-slate-800 active:bg-slate-900 text-white hover:text-black cursor-pointer";

// Botones Completos
export const BotonVerdeRedondeado = `${BotonRedondeado}, ${ColorVerde}`;
export const BotonVerdeClasico = `${BotonClasico}, ${ColorVerde}`;

export const BotonPrimaryRedondeado = `${BotonRedondeado}, ${ColorPrimary}`;
export const BotonPrimaryClasico = `${BotonClasico}, ${ColorPrimary}`;
export const BotonPrimaryClasicoSinZoom = `${BotonClasicoSinZoom}, ${ColorPrimary}`;

export const BotonNegroRedondeado = `${BotonRedondeado}, ${ColorNegro}`;
export const BotonNegroClasico = `${BotonClasico}, ${ColorNegro}`;
export const BotonNegroClasicoSinZoom = `${BotonClasicoSinZoom}, ${ColorNegro}`;

export const BotonBlancoRedondeado = `${BotonRedondeado}, ${ColorBlanco}`;
export const BotonBlancoClasico = `${BotonClasico}, ${ColorBlanco}`;
export const BotonBlancoClasicoSinZoom = `${BotonClasicoSinZoom}, ${ColorBlanco}`;

export const BotonRojoRedondeado = `${BotonRedondeado}, ${ColorRojo}`;
export const BotonRojoClasico = `${BotonClasico}, ${ColorRojo}`;

export const BotonAzulRedondeado = `${BotonRedondeado}, ${ColorAzul}`;
export const BotonAzulClasico = `${BotonClasico}, ${ColorAzul}`;
